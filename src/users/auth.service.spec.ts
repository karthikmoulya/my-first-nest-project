...
describe('AuthService', () => {
  ...
  it('throws an error if user signs up email that in use', async () => {
  ...
  it('throws if signin is called with an unused email', async () => {
    await expect(service.signin('test@test.com', 'tktk123')).rejects.toThrow(
      NotFoundException,
    );
  });

  it('throws if an invalid password is provided', async () => {
    await service.signup('adc@abc.com', 'password');
    await expect(service.signin('adc@abc.com', '12345')).rejects.toThrow(
      BadRequestException,
    );
  });
